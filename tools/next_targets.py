...
def _load_wall_predictions(version: str) -> dict[str, list[dict]]:
    """Brief 189: load `tools/predict_walls.py`'s JSON output if it
    exists, keyed `<module>:0x<addr>` → list of `{"id", "cue"}`
    dicts. Empty dict if the predictions file doesn't exist —
    callers degrade gracefully to no-wall-annotation behaviour.

    Re-run `python tools/predict_walls.py --version <region>` to
    refresh the predictions database after shipping matches.
    """
    pred_path = ROOT / "build" / version / "analysis" / "wall_predictions.json"
...
def _annotate_reason_with_walls(
    target: Target,
    predictions: dict[str, list[dict]],
) -> str:
    """Suffix `target.reason` with a `[walls: …]` annotation when
    `predict_walls.py` reports any walls for the target. Brief 189
    Part 3 — replaces the old `[NB: <module> module checksum
    failing]` annotation (modules are 27/27 OK since brief 140).

    No-op when no predictions exist or the function isn't in the
    predictions database.
    """
    s = target.symbol
    key = f"{s.module}:0x{s.addr:08x}"
    walls = predictions.get(key, [])
    if not walls:
        return target.reason
    wall_ids = ", ".join(w["id"] for w in walls)
    return f"{target.reason}  [walls: {wall_ids}]"
...
def main() -> int:
    ...
    ap.add_argument("--no-walls", action="store_true",
                    help="Skip merging wall predictions from "
                         "build/<region>/analysis/wall_predictions.json "
                         "(brief 189). Default: merge when the file "
                         "exists.")
    args = ap.parse_args()
    ...
    # Brief 189 Part 3: merge wall predictions into each target's
    # `reason` string when the predictions database exists. The
    # database is generated by `tools/predict_walls.py` and lives
    # at build/<region>/analysis/wall_predictions.json (gitignored
    # under build/). The annotation surfaces as `[walls: StyleA,
    # C-15]` etc. — `next_targets.md`'s Reason column carries the
    # hint into the queue + JSON consumers see it too.
    if not args.no_walls:
        predictions = _load_wall_predictions(args.version)
        if predictions:
            for t in targets:
                t.reason = _annotate_reason_with_walls(t, predictions)
