...
Legacy briefs (numbered `NNN-slug.md`, pre-2026-09-21 framework adoption)
were moved verbatim into `docs/briefs/archive/legacy/` when the project
adopted the agentic framework; `docs/briefs/README.md` is now the
framework's own brief-lifecycle document. This generator and its index
stay pointed at the archive so the freshness check that used to gate
`docs/briefs/README.md` keeps gating the same content in its new home.
...
alongside the brief itself.
    ...
    # → writes docs/briefs/archive/legacy/README.md
...
BRIEFS_DIR = ROOT / "docs" / "briefs" / "archive" / "legacy"
INDEX_PATH = BRIEFS_DIR / "README.md"

READ_ONLY_NOTICE = (
    "> **Read-only archive.** These briefs predate the project's "
    "2026-09-21 adoption of the agentic framework and are kept verbatim "
    "for history. New work is tracked via `docs/briefs/README.md`, "
    "`docs/briefs/active.md`, `docs/briefs/delivered/` and "
    "`docs/briefs/archive/`, per `AGENTS.md`. Do not add new numbered "
    "briefs here."
)
...
def render_index(briefs: list[dict]) -> str:
    ...
    lines += [
        "",
        "## This archive does not take new briefs",
        "",
        "New briefs are written by Brain per `docs/agents/briefs.md` and move",
        "through `docs/briefs/active.md` → `delivered/` → `archive/`. This",
        "generator only keeps the legacy index above in sync with the files",
