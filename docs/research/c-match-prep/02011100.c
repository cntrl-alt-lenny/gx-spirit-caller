/* CAMPAIGN-PREP candidate for func_02011100 (main, class A) — brief 494.
 ...
 *   confidence: high
 */
/* func_02011100: if shift<=0 return; else left-shift each of 9 words
 * (offsets 0..0x20) in place by `shift`. Leaf. */

void func_02011100(int *p, int shift) {
    if (shift <= 0) return;
    p[0] <<= shift;
    p[1] <<= shift;
    p[2] <<= shift;
