/* CAMPAIGN-PREP candidate for func_020920bc (main, class C, MED tier) — brief 496.
...
void func_020920bc(Node *node){
    Node *cur;
    Node *prev = 0;
    Node *head = *(Node **)(data_021a63d0 + 0x8);
    cur = head;
    while (cur != 0 && cur->key < node->key) { prev = cur; cur = cur->next; }
    if (prev == 0) { node->next = head; *(Node **)(data_021a63d0 + 0x8) = node; }
