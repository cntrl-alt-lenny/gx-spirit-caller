/* CAMPAIGN-PREP candidate for func_02250350 (ov002, class A) — brief 494.
 ...
 *   confidence: high
 */
/* func_ov002_02250350: per-player slot scan. For each slot whose id maps via
 * func_0202b878 to 0xf, mark via the list sink. */
#include "ov002_core.h"

extern char data_ov002_022cf180[];
extern int func_0202b878(int id);

void func_ov002_02250350(int player) {
    int i;
    ...
    for (i = 0; i < *(unsigned int *)(base + 0x14); i++) {
        if (func_0202b878(slots[i].id) == 0xf)
