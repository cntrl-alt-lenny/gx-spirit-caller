...
|---|---|---|
| **Reg-alloc / scratch-register choice (NARROWED, not a blanket wall — see below)** | Same instructions and values, but mwcc chooses a different physical scratch register. R&D swarm r6 + this session's reproduction: **3 of 7 named brief-641/P-4 instances byte-matched** once routed to the correct compiler tier (plain `.c`/`.legacy.c`/`.thumb.c` — the b665/b667 routing-tier pattern) and, for one, forcing genuine store materialization with `volatile` on an otherwise-dead local. The r6 swarm independently verified 6 of 8 + P-4's `func_02084ac4` (7 total); this session reproduced 3 of those 7 from scratch (the swarm's own scratch source was not preserved) — `func_02084ac4`, `func_02096040`, `func_ov004_021de264`. The remaining 4 (`func_020a71e4`, `func_020a724c`, `func_0207e214`, `func_ov004_021dbe68`) still show pure register-letter residue after tier-routing + `volatile` attempts — **do not park reg-alloc-shaped candidates on sight**; try correct-tier routing and a `volatile` local first, and treat only a residue that survives *both* as the genuine wall. `func_020b3850` and `func_0208b1ac` are confirmed genuine survivors (do not re-attempt). | [`brief-641-final47.md`](../brief-641-final47.md), [`brief-650-ov002-sweep.md`](../brief-650-ov002-sweep.md), `rnd-swarm-2026-07-23-r6.md` bet 1, `codegen-walls.md` P-4 |
