...
|---|---|---|
| **Reg-alloc / scratch-register choice** | Same instructions and values, but mwcc chooses a different physical scratch register. **0 W / 7 named final47 wall instances**; every source lever tried there (names, declaration order, dummy parameters, statement order, explicit pointers) had zero effect. Ov002 independently reconfirmed the signature. | [`brief-641-final47.md`](../brief-641-final47.md), [`brief-650-ov002-sweep.md`](../brief-650-ov002-sweep.md), `codegen-walls.md` P-4/P-11/P-12/P-15 |
...
| 13 | Branch polarity controls predication per guard | Choose `goto`/shared-tail or plain return independently at each guard; different guards in one function can need opposite shapes. | [`brief-650-ov002-sweep.md`](../brief-650-ov002-sweep.md), `codegen-walls.md` C-1/C-13 | **1 / 0** — one ov002 ship explicitly credits the lever; one 93% near-miss records the same fix |
...
| 28 | `*.legacy.c` routing for the compiler-tier peephole | Use mwcc 1.2/sp2p3 when the 2.0 tier folds the target's prologue, pool, MMIO, or thunk shape differently. | `codegen-walls.md` C-15/C-20/C-24/C-26; `style-a-epilogue.md`; queue item `cm-epilogue-wall` | **≥8 / 0 enumerated** — explicit byte-identical routing examples across C-15, C-20, C-24, C-26, plus 2 more this brief (`func_020915e4`, `func_0206eecc` — separate `stmfd{lr};sub sp,#4` + `pop{regs,lr};bx lr` epilogue mismatch that 3 prior briefs, including a `volatile`-local attempt, had misdiagnosed as uncontrollable) |
| 29 | `*.legacy_sp3.c` routing | Use the 1.2/sp3 tier for `stmfd {lr}; sub sp,#4` and related pool-dedup/prologue shapes. | `codegen-walls.md` C-24; `recipe-gotchas.md` Gotcha 10; queue item `cm-epilogue-wall` | **≥3 / 0** — brief 242 plus the C-24 worked example, plus `func_020458d8` this brief (`stmfd{lr};sub sp,#4` + fused `pop{regs,pc}` epilogue, all 4 branch bodies were already known to match — see lever-payoff.md's routing table: prologue+epilogue shape alone discriminates the tier, no body change needed) |
