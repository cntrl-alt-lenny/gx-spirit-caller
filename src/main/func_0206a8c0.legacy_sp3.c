/* CAMPAIGN-PREP candidate for func_0206a8c0 (main, class A) — brief 494.
 ...
 *   confidence: high
 */
/* func_0206a8c0 -> route as func_0206a8c0.legacy_sp3.c (mwcc 1.2/sp3).
 * Skip-token loop: while still bytes remaining and current byte nonzero,
 * advance past two sub-tokens via func_0206b44c (negative return = error).
...
int func_0206a8c0(signed char *s, int n) {
    while (n > 0 && *s != 0) {
        int d = func_0206b44c(s, n);
        if (d < 0) return 0;
        n -= d; s += d;
        d = func_0206b44c(s, n);
        if (d < 0) return 0;
