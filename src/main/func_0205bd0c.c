/* CAMPAIGN-PREP candidate for func_0205bd0c (main, class A) — brief 494.
 ...
 *   confidence: high
 */
/* func_0205bd0c: search list (head=(*pp)->+0x424, link +0x20) for a
...
int func_0205bd0c(Root2 **pp, Node2 **out, int key) {
    Node2 *n = (*pp)->head;
    while (n != 0) {
        if (n->key == key) {
            if (out != 0) *out = n;
