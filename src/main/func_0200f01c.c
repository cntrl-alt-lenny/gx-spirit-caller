/* CAMPAIGN-PREP candidate for func_0200f01c (main, class B) — brief 494.
 ...
 *   confidence: high
 */
/* func_0200f01c: null-guard, then rewrite flag word at +0x60:
 *   if (p == 0) return 0;
 *   v = [p+0x60];
 *   v = (v & ~4) | 8;        (bic #4; orr #8)
...
int func_0200f01c(struct Obj0200f01c *p) {
    unsigned int v;
    if (p == 0) return 0;
    v = p->flags;
    v = (v & ~4u) | 8u;
