/* func_02037c54: sibling of func_0203c5e4 — same interpolation setup,
 * but selects the anim struct by integer index into a global table
 * (`data_0219b3d0`, 0x10 bytes per entry), and clamps a zero-frame
 * caller to 1 frame rather than returning early.
 *
 * Disassembly (build/eur/asm/_dsd_gap@main_8.s):
 *
 *     stmdb sp!, {r4, lr}
 *     ldr   r3, .L_02037c9c         ; r3 = &data_0219b3d0[0]
 *     cmp   r2, #0x0                ; compare arg frames
 *     add   r4, r3, r0, lsl #0x4    ; r4 = &data_0219b3d0[index]
 ...
 *     ldmia sp!, {r4, pc}
 *     .L_02037c9c: .word data_0219b3d0
 *
 * 18 ARM instructions + 1 literal-pool word, 0x4c bytes.
 *
 * Differences from func_0203c5e4:
 *   (1) Takes an integer `index` instead of a pointer. `entry` is
 *       computed as `data_0219b3d0 + index * 0x10` — hence the need
...
void func_02037c54(int index, int target, unsigned short frames) {
    anim_linear_t *entry = &data_0219b3d0[index];
    int target_q24_8;
    int step;
