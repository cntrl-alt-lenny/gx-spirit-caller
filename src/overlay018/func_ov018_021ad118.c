/* func_ov018_021ad118: tone-shift a run of `count` RGB555 cells (low half of
 * `packed` = count, high half = scale). Pass 1 decomposes each source cell via
 * func_ov018_021ace68 and tracks the minimum of its third component. Pass 2
 * re-decomposes, knocks (min*scale)>>8 off that component, recombines via
 * func_ov018_021acf80, and writes the result to dst.
 */

extern void func_ov018_021ace68(int value, int *out0, int *out1, int *out2);
extern int func_ov018_021acf80(int a, int b, int c);

void func_ov018_021ad118(unsigned short *src, unsigned short *dst, unsigned int packed) {
    int c0;
    int c1;
    int c2;
    int i;
