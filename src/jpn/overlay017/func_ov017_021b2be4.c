/* func_ov017_021b2be4: hit-test the touch point against rect r (x,y u16 +
 * w,h u8) — returns 1 if the pen is down and inside.
 *
 *     add r0,sp,#4; add r1,sp,#0; bl func_020060f4   ; x->[sp+4], y->[sp+0]
 *     bl func_02006190; cmp r0,#0; beq .fail
...
int func_ov017_021b2be4(Rect *r) {
    int x, y;
    func_020060f4(&x, &y);
    if (func_02006190() != 0 &&
        r->x <= x && r->x + r->w > x &&
