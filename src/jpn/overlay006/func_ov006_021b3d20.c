/* func_ov006_021b3d20: per-state "enter" (long) — kick the per-state full-init
 * (arg 1), seed the first subobject with (mode 3, 0), then run the seven fixed
 * per-subobject setup calls and advance the state word to 4. Returns 1. 0x80
 * enter family, sibling of 021b387c / 021b4db8 (ov006_core.h). */
extern void func_ov006_021b3a40(int);
...
int func_ov006_021b3d20(void) {
    func_ov006_021b3a40(1);
    func_ov006_021b9df8(data_ov006_0225c3bc, 3, 0);
