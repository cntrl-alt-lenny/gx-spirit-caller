/* func_ov002_021e770c: 0x868 sub-row accessor wrapper (Cluster A). Guards on
 * the opponent check (player must differ from self->b0), idx range, and the
 * sub-row's f30 id, then forwards (self, player, idx) to func_ov002_021c1e10.
...
int func_ov002_021e770c(struct self_t *self, int player, int idx) {
    struct subrow *sr;
    if (player == self->b0)
        return 0;
    if (idx >= 5)
