/* func_ov002_021ef6c4: repeated obj+2 bitfield extracts (orig RELOADS ldrh
...
extern int func_ov002_021bc1a8(int side, int state);
extern int func_ov002_021bc538(int owner);
extern int func_ov002_021bc7e8(int owner, int side, int state);

int func_ov002_021ef6c4(struct Obj021ef7b4 *obj, int side, int state)
{
    int owner;
    ...
    if (func_ov002_021bc538(obj->f2.owner) != 0)
        goto return_true;
    if (obj->f2.owner != side)
        return 0;
    if (func_ov002_021bc1a8(side, state) == 0)
        return 0;
return_true:
    return 1;
}
