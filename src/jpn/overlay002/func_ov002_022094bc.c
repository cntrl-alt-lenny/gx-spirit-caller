...
extern int func_ov002_021c8860(int player, int i);

int func_ov002_022094bc(void *self_raw) {
    struct SelfF2 *self2;
    int i;
    int count = 0;
    int row_offset;

    if (data_ov002_022d008c.f_cf8 != 2) {
        return 0;
    }
    ...
    for (i = 0, row_offset = 0; i <= 4; i++, row_offset += 0x14) {
        ...
        char *row = data_ov002_022cf08c + (player & 1) * 0x868 + row_offset;
        ...
        if (func_ov002_021c8860(player, i) == 0x12) {
            count++;
        }
    }
