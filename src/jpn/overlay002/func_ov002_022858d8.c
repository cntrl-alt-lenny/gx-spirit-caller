/* func_ov002_022858d8 (ov002): for each of 5 slots of player idx's row
 * (stride 0x868, row+i*0x14): if slot.id:13 != 0, run a guard chain
 * (func_ov002_021c9ee4 / func_0202f994 / f36 nonzero / func_ov002_021bd868);
...
extern int func_ov002_021bd868(int idx, int i);
extern int func_ov002_021c9ee4(int idx);
...
void func_ov002_022858d8(int idx)
{
    int i;
    unsigned int id;
    Slot022859e8 *slot = (Slot022859e8 *)(data_ov002_022cf08c + (idx & 1) * 0x868 + 0x30);
    char *row = data_ov002_022cf08c + (idx & 1) * 0x868;
    ...
    for (i = 0; i < 5; i++, slot++, row += 0x14) {
        ...
        if (*(unsigned short *)(row + 0x36) == 0)
            continue;
        if (func_ov002_021bd868(idx, i) != 0) {
