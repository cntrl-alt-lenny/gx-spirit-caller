/* CAMPAIGN-PREP candidate for func_02073cb8 (main, class A) — brief 494.
 ...
 *   confidence: high
 */
/* CAMPAIGN-PREP candidate for func_02073cb8 (main, class A).
 ...
 */
/* func_02073cb8(a, b): for i in 0..2 if (a[i]!=b[i]) return 1; return 0.
 * u16 elements, *p++ post-increment addressing. */

int func_02073cb8(const unsigned short *a, const unsigned short *b) {
    int i;
    for (i = 0; i < 3; i++) {
        if (*a++ != *b++)
            return 1;
