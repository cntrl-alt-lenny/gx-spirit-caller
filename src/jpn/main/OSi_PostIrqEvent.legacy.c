 ...
 * post-action flag (when non-zero) or hands off to func_020904f4(mask).
 *
 *     stmdb sp!, {r4, r5, lr}
 *     sub   sp, sp, #0x4
 *     mov   r1, #0xc
 *     mul   r4, r0, r1               ; r4 = idx * 12  (struct stride)
 *     ldr   r2, .L_CB_TABLE          ; r2 = &data_021a6274
 *     ldr   r3, .L_SHIFT_TABLE       ; r3 = &data_021023c0
 ...
 *     mov   r0, r5
 *     bl    func_020904f4            ; else hand off to dispatch
 *     add   sp, sp, #0x4
 *     ldmfd sp!, {r4, r5, lr}
 *     bx    lr
 *   .L_CB_TABLE:    .word data_021a6274
 *   .L_SHIFT_TABLE: .word data_021023c0
 *   .L_ARG_TABLE:   .word data_021a627c
 *   .L_DTCM_BASE:   .word 0x027e0000
 *   .L_FLAG_TABLE:  .word data_021a6278
 *
 * **C-23 wall — clustered pool window.** The three pool words
 * `data_021a6274 / data_021a6278 / data_021a627c` are 4 B apart but
 ...
 * (`&data_021a6274[idx * 12]`) so mwcc emits `mul #12` directly instead
 ...
 * Writing it as `1 << data_021023c0[idx]` (load + shift inline) anchors
 * `mask` in callee-saved r5 from the first use, matching orig.
 */

typedef void (*OSi_IrqHandler)(int arg);

extern unsigned char  data_021a6274[];      /* irq callback table, stride 12 */
extern unsigned char  data_021a6278[];      /* irq post-action flag table */
extern unsigned char  data_021a627c[];      /* irq callback-arg table */
extern unsigned short data_021023c0[];      /* slot → bit-shift table */
extern int            func_020904f4(int mask);

int OSi_PostIrqEvent(int idx) {
    int off = idx * 12;
    int mask = 1 << data_021023c0[idx];
    OSi_IrqHandler cb = *(OSi_IrqHandler *)&data_021a6274[off];
    *(int *)&data_021a6274[off] = 0;
    if (cb != 0) {
        cb(*(int *)&data_021a627c[off]);
    }
    {
    ...
    {
        int v = *(int *)&data_021a6278[off];
        if (v != 0) return v;
    }
    return func_020904f4(mask);
}
