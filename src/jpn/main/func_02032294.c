/* CAMPAIGN-PREP candidate for func_02032294 (main, class C, MED tier) — brief 496.
 ...
 *   confidence: med
 */
/* func_02032294: walk a list; for each node, if (node->f10 & 2) and
 * node->f14==0, call func_02031f1c(node, 2) then func_02031e68(head, node).
 * head=r0(r7); first node = head->f8; next = node->f4; stop when next==0.
 * Early-out if head==0 or head->f8==0.
 */

extern void func_02031f1c(void *node, int k);
extern void func_02031e68(void *head, void *node);
...
void func_02032294(struct LNode_020322e8 *head) {
    ...
    do {
        ...
        if ((node->f10 & 2) && node->f14 == 0) {
            func_02031f1c(node, 2);
            func_02031e68(head, node);
        }
        node = next;
    } while (node != 0);
}
