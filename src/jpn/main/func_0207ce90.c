/* CAMPAIGN-PREP candidate for func_0207ce90 (main, class A) — brief 494.
 ...
 *   confidence: high
 */
/* func_0207ce90: 6-byte equality compare, clean leaf (no frame). Returns 1
...
int func_0207ce90(const unsigned char *a, const unsigned char *b) {
    int i;
    for (i = 0; i < 6; i++) {
        if (a[i] != b[i]) {
            return 0;
