/* CAMPAIGN-PREP candidate for func_02076bd8 (main, class C, MED tier) — brief 496.
 ...
 *   confidence: med
 */
/* func_02076bd8: linear scan of `count` records, big-endian key match.
...
int func_02076bd8(unsigned char *p, int count, int stride, int target) {
    int i;
    for (i = 0; i < count; i++) {
        int val = (p[0] << 8) + p[1];
        if (stride == 3)
