/* CAMPAIGN-PREP candidate for func_02091924 (main, class C, MED tier) — brief 496.
...
extern struct Node *func_020920c0(void *);
...
void func_02091924(Container *c)
{
    int irq = OS_DisableIrq();
    if (c->head != 0) {
        while (c->head != 0) {
            Node *n = func_020920c0(c);
            n->f64 = 1;
            n->f78 = 0;
            n->f80 = 0;
            n->f7c = n->f80;
