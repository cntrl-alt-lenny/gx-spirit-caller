/* func_ov002_021ae400: guarded event-trigger hub (64 band-callers).
 * If the per-slot done-flag data_ov002_022cd744[a] is unset, dispatch
 * event 0x31 (payload b) through the central event dispatcher
 * func_ov002_0229ade0; otherwise no-op. Sibling func_ov002_021ae484 is
 * the unguarded form (dispatch(0x31, 1, x, 0)); this one gates on the
 * done-flag array and passes code-2 = 0.
 *
 *     push  {r3, lr}
 *     ldr   r3, =data_ov002_022cd744
 *     mov   r2, r1
 *     ldr   r0, [r3, r0, lsl #2]
 *     cmp   r0, #0
...
void func_ov002_021ae400(int a, int b) {
    if (data_ov002_022cd744[a] != 0) return;
    func_ov002_0229ade0(0x31, 0, b, 0);
}
