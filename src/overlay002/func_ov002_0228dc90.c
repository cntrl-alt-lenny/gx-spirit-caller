/* func_ov002_0228dc90: node = (CE288->f_48c - 0x18) as a list-node pointer;
 * bail (return 0) if node is null or if node->b0 == arg0. Else scan
 * idx=0..node->cnt-1 (8-bit count field at +6) via
 * func_ov002_0223de94(node, idx) -> packed (lo=byte0, hi=byte1); return 1
...
int func_ov002_0228dc90(int arg0, int arg1) {
    struct DcNode *node = (struct DcNode *)(CE288->f_48c - 0x18);
    int i;
    if (node == 0) goto fail;
    if (node->b0 == arg0) goto fail;
    for (i = 0; i < node->cnt; i++) {
        unsigned int packed = func_ov002_0223de94(node, i);
