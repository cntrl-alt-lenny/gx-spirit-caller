...
extern int func_ov002_021b3618(int player, int idx);
extern void func_ov002_021de4b0(struct Ov002Self *self, int a, int b);

int func_ov002_02239fe0(struct Ov002Self *self)
{
    unsigned short raw = *(unsigned short *)((char *)self + 2);
    unsigned int field = (unsigned int)(raw << 26) >> 27;
    Row420 *row_a = (Row420 *)(data_ov002_022cd420 + D016C->f_cec * 0x38);
    Row420 *row_b = (Row420 *)(data_ov002_022cd420 + (1 - D016C->f_cec) * 0x38);
    int result;

    result = func_ov002_021b3618(self->b0, field);
    ...
    }
    {
        ...
        if (result != (unsigned short)packed) {
            ...
            if (row_b->fc == slot->flagBit + (slot->field8 << 1)) {
                func_ov002_021de4b0(self, row_b->f0, row_b->f4);
            }
        }
    }
    return 0;
