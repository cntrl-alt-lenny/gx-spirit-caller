/* func_ov002_021bd274: for i=0..9 over player's cf16c row, if slot.id!=0
 * and the +0x38 flag u16!=0: for i<5 additionally require
 * func_ov002_021b8fcc(player,i); then count func_0202b878(id)==0x17.
...
int func_ov002_021bd274(int player) {
    int i;
    int result = 0;
    char *id_ptr = data_ov002_022cf16c + (player & 1) * 0x868 + 0x30;
    ...
    for (i = 0; i < 10; i++, id_ptr += 0x14, row += 0x14) {
        if (((struct Ov002Slot *)id_ptr)->id != 0 && *(unsigned short *)(row + 0x38) != 0) {
            if (i < 5) {
                if (func_ov002_021b8fcc(player, i) == 0)
                    continue;
            }
            {
