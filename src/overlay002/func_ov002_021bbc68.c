/* func_ov002_021bbc68: for each i=0..4 (excluding i==a2), if the
 ...
 * call func_ov002_021c93cc(a0,i,a1) and count nonzero results.
 */
struct Ov002Slot {
    unsigned int id : 13;
};
...
int func_ov002_021bbc68(int a0, int a1, int a2) {
    int i;
    int count;
    struct Ov002Slot *slot;
    char *entry;

    entry = (char *)data_ov002_022cf16c + (a0 & 1) * 0x868;
    ...
    for (i = 0; i < 5; i++, slot = (struct Ov002Slot *)((char *)slot + 20), entry += 20) {
        ...
        if (slot->id != 0 && *(unsigned short *)(entry + 0x38) != 0) {
            ...
            if (!(((flags >> 1) | (flags >> 2)) & 1)) {
                if (func_ov002_021c93cc(a0, i, a1) != 0) {
                    count++;
                }
            }
        }
