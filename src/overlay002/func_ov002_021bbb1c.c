/* func_ov002_021bbb1c: scan sub-rows i=5..10 of arg0's per-player row; for
 * each where func_ov002_021b9ecc(arg0,i) matches arg1 and a bit6/bit(1|2)
...
int func_ov002_021bbb1c(int arg0, int arg1, int arg2) {
    int i;
    int count = 0;
    char *p = data_ov002_022cf16c + (arg0 & 1) * 0x868 + 0x64;
    for (i = 5; i <= 10; i++, p += 0x14) {
        int r = func_ov002_021b9ecc(arg0, i);
        if (r <= 0)
            continue;
        if (r != arg1)
            continue;
