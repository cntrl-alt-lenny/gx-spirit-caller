/* func_ov002_0220b488 (ov002).
 *
 * Guard chain on item->id / item->f2 bitfields, then forwards a
 * derived (player, 1, filt, mapped) tuple to func_ov002_021ff020, and
 * finally scans item's slot list (via func_ov002_0223de94) for a slot
 ...
 *    idiom against the SAME callee, func_0202e234). offset 0x6 (u16,
 ...
 *    src/overlay002/func_ov002_021ff320.c exactly (same self pointer,
 ...
 *    func_ov002_021ff020 per docs/research/dossiers/021ff020.md.
 *
 * Callee signature notes:
 *  - func_0202de9c and func_ov002_021ff320 are each called here with
 *    one MORE argument than their own shipped definitions declare
 *    (src/main/func_0202de9c.c: `int func_0202de9c(void)`;
 *    src/overlay002/func_ov002_021ff320.c: `int
 *    func_ov002_021ff320(struct Func021ff320Self *self)`). The extra
 ...
 *    `func_0202de9c(v)` with one int argument. The extra argument is
...
int func_ov002_0220b488(SelfT *self, ItemT *item)
{
    ...
    if (IsValue12D3Or147E(item->id) != 0)
        return 0;
    if (func_ov002_021ff320(self, item) == 0)
        return 0;

    if (func_0202e234(item->id) == 0)
        goto check_field2;
    if (item->f2.field5 < 5)
        goto after_guard;
...
check_field2:
    if (item->f2.field2 != 0)
        return 0;
after_guard:
    mapped = func_0202de9c(item->id);
    if (func_0202e234(item->id) != 0) {
        if (item->f2.field5 >= 5 && item->f2.field5 <= 0xa)
            mapped = 7;
    }

    if (func_ov002_021ff020(self->f2.player, 1, self->f4.filt, mapped) == 0)
        return 0;

    for (i = 0; i < (int)((unsigned int)(item->f6 << 24) >> 24); i++) {
        got = func_ov002_0223de94(item, i);
