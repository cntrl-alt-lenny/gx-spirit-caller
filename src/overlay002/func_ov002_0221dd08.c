...
extern void func_ov002_021df818(void *self, int player, int ms);
...
int func_ov002_0221dd08(void *self) {
    ...
    struct Ov002LevelWord *w = (struct Ov002LevelWord *)((data_ov002_022cf16c + (player & 1) * 0x868 + 0x30) + idx * 0x14);
    int computed = w->half + (w->level << 1);
    if (val9 != computed) {
        return 0;
    }
    if (*(u16 *)((char *)self + 8) != 0) {
        func_ov002_021df818(self, 1 - player, 500);
    }
    return 0;
}
