/* func_ov002_021f31bc: guard chain (player!=self->b0, 5<=idx<10,
 * slot.id!=0) to a shared "return 0" tail; else forward to
 * func_ov002_021c2084 as a boolean. */
#include "ov002_core.h"

int func_ov002_021f31bc(struct Ov002Self *self, int player, int idx) {
    if (player == self->b0) {
        return 0;
    }
    if (idx < 5) {
