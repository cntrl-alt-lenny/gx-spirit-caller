/* func_ov002_021ba6cc: for player (self&1), if the cf17c counter is
 * positive, walk the row+0x260 packed-id array, calling
 * func_0202df78(id, arg1) for each of the row+0x10 (reloaded each
...
int func_ov002_021ba6cc(int self, int arg1) {
    int off = (self & 1) * 0x868;
    unsigned int n = *(unsigned int *)((char *)data_ov002_022cf17c + off);
    int i;
    int count = 0;
    i = count;
    if (i < n) {
        ...
        do {
            if (func_0202df78(p->id, arg1) != 0) count++;
            i++;
            p++;
