/* data_ov004_02206760 (1024 bytes, 4-aligned): wave-13 retype, actually
 * applied during wave 14 startup after discovery that the original wave-13
 * commit (a449bc562/PR #1383) documented and gated this exact change but
 * never made it -- a real process bug (the commit's own file list omitted
 * this path), not a byte-content error. See
 * feedback_claimed-change-was-never-written for the postmortem.
 *
 * Real AES **Td2** inverse-cipher round table (256 entries), verified
 * byte-for-byte against an independently-computed reference table (built
 * from the inverse S-box and inverse-MixColumns coefficients 14,9,13,11 --
 * standard AES decryption, not encryption). relocs.txt: the sole consumer
 * is a single fixed-offset base-address load from inside the still-opaque
 * `data_ov004_021ded69` blob; element count/stride proven directly from
 * that raw consumer's own disassembly (an explicit `LSL r3,r1,#2` --  x4
 * stride -- immediately preceded by `AND r1,r2,#0xff` -- index masked to
 * 0-255 -- before the indexed `LDR r5,[r1,r3]`). Sibling of the
 * data_ov004_02206738 Rcon table (WEAK, declined) and 8 more 0x400-spaced
 * AES tables not yet individually carved. Content generated via a Python
 * script reading extract/eur/arm9_overlays/ov004.bin directly (never
 * hand-transcribed) and round-trip-verified byte-exact before writing.
 */

const unsigned int data_ov004_02206760[256] = {
