/* func_0202b060: card-ID → u16 lookup with sentinel. For IDs below
 * 0xfa0 returns 0 (invalid / reserved range); above that, indexes into
 * a u16 table hung off `data_0219a85c.p0->table_34` by `(id - 0xfa0)`.
 *
 * Anchor of the mega-cascade cluster found by #207's
 * `find_mega_cascades.py`: 9 direct callers, 41 indirect at depth 4,
 * 50 functions total. Sibling lookups (`func_0202b08c`, `func_0202b0ac`,
 * `func_0202b2c8`, `func_0202b2e8`, …) all go through the same
 * `data_0219a85c.p0->...` shape with different field offsets.
 *
 *     cmp   r0, #0xfa0
 *     movlt r0, #0x0
 *     bxlt  lr
 *     ldr   r1, .L_0202b0dc          ; &data_0219a85c
 *     sub   r0, r0, #0xfa0
 *     ldr   r1, [r1, #0x0]           ; r1 = data_0219a85c.p0
...
typedef struct card_ctx {
    ...
    /* rest: 0x8 = void **slot (see func_0202c460) — not modelled here */
} card_ctx_t;

extern card_ctx_t data_0219a85c;

unsigned short func_0202b060(int id) {
    if (id < 0xfa0) return 0;
    return data_0219a85c.p0->table_34[id - 0xfa0];
}
