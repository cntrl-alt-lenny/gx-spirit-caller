/* CAMPAIGN-PREP candidate for func_0200a894 (main, class D, MED tier) — brief 496.
...
extern unsigned short data_0210672e[];

struct nib { unsigned short lo : 4; unsigned short hi : 4; unsigned short rest : 8; };

int func_0200a894(int bit)
{
    struct nib *src = (struct nib *)((char *)data_0210665c + 0xd2);
    int ret = 0;
    ...
    if (ret) {
        struct nib *dst = (struct nib *)data_0210672e;
        dst->hi |= 1 << bit;
    }
    return ret;
}
