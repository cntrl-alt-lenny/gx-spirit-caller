/* CAMPAIGN-PREP candidate for func_0206aac8 (main, class A) — brief 494.
 ...
 *   confidence: high
 */
/* func_0206aac8 -> route as func_0206aac8.legacy_sp3.c (mwcc 1.2/sp3).
 * Counted loop: for i in [0, p->f480), release p->arr[i] (the int array at
 * offset 0x84) via func_0206b408(p, p->arr[i]); then clear p->f480.
...
void func_0206aac8(struct S *p) {
    int i;
    for (i = 0; i < p->f480; i++) {
        func_0206b408(p, p->arr[i]);
    }
    p->f480 = 0;
}
