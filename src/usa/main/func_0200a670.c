/* CAMPAIGN-PREP candidate for func_0200a670 (main, class D, MED tier) — brief 496.
 ...
 *   confidence: med
 */
extern unsigned short data_0210665c[];
extern unsigned short data_0210672c[];

struct nib { unsigned short lo : 4; unsigned short hi : 4; unsigned short rest : 8; };

int func_0200a670(int bit)
{
    struct nib *src = (struct nib *)((char *)data_0210665c + 0xd0);
    int ret = 0;
    ...
    if (ret) {
        struct nib *dst = (struct nib *)data_0210672c;
        dst->hi |= 1 << bit;
    }
    return ret;
}
