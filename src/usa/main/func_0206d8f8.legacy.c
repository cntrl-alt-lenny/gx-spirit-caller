/* func_0206d8f8: validates `a` against several bitfield/state checks
 * (each failing with a distinct negative error code), then dispatches
 * to func_020925a4 or func_020926d0 (both on a->field_68+0xe0)
 * depending on a mix of the 6th argument's bit 2 and a->field_72,
 * before finally calling func_0206d7e0(a,b,c,d,e,dispatch_flag) and
 * returning its result -- with a trailing, result-discarded call to
 * func_02092660(a->field_68+0xe0) squeezed in before the return.
 *
 ...
 * matching (C-55): the fallthrough block is func_020925a4, not
 * func_020926d0 -- a combined `(f&4)||(field_72==0)` if/goto produces
...
extern int func_020925a4(void *p);
extern int func_020926d0(void *p);
extern int func_02092660(void *p);
extern int func_0206d7e0(Obj_0206d96c_t *a, int b, int c, int d, int e, int flag);

int func_0206d8f8(Obj_0206d96c_t *a, int b, int c, int d, int e, int f) {
    int ok;
    int result;
    void *p;
    int flag;
...
L_290:
    p = base68 + 0xe0;
    result = func_020926d0(p);
    flag = 1;

L_29c:
    result = func_0206d7e0(a, b, c, d, e, flag);
    p = base68 + 0xe0;
    func_02092660(p);
    return result;
}
