/* CAMPAIGN-PREP candidate for func_0201a4a8 (main, class C, MED tier) — brief 496.
 ...
 *   confidence: med
 */
/* CAMPAIGN-PREP candidate for func_0201a4a8 (main, class C) — UNVERIFIED build-free draft.
 * Guard chain; GetSystemWork bound in r4 across the function (reloaded only at the +0x900 tail).
 *   sys = GetSystemWork();
 *   if (func_020190b8()) {                 // .L_80 path when func_020190b8()==0 falls to else? NO:
 *       if (func_02013980() >= 2) goto bit; // bge .L_80
 *       return 1;
 *   } else {                               // .L_70
 *       if (func_020190f0() < 0xa8) return 2;   // movlt+conditional return; else fall through
 *   }
...
int func_0201a4a8(void) {
    char *r4 = GetSystemWork();
    if (func_020190b8() != 0) {
        if (func_02013980() < 2)
            return 1;
    } else {
        if (func_020190f0() < 0xa8)
            return 2;
    }
