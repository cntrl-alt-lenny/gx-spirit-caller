/* func_02037d70: mode-dispatch scan over the 28-entry data_0219b680 table
 * (0x78 stride, Entry68-shaped per func_02038118.c ground truth). Modes
 * 0-5 loop every entry, skip inactive ones (signed byte +0x6a < 0), apply
 * a per-mode flag-bit guard on the +0x34 word, and call
 * func_02037cd8(entry, arg1, arg2) for entries that pass. Any other mode
 * (arg0 - 0x8f00000 out of [0,5]) resolves a caller-context entry via
 * func_02037b08 and calls func_02037cd8 once with it, or returns 0 if
 * unresolved.
 */
...
extern Entry68 *func_02037b08(int id);
extern int func_02037cd8(void *entry, void *arg1, void *arg2);
extern unsigned char data_0219b680[];
extern unsigned char data_0219b200[];

int func_02037d70(int modeArg, void *arg1, void *arg2) {
    unsigned int mode = modeArg - 0x8f00000;
    unsigned char *entry = data_0219b680;
    int idx;
    ...
    case 5:
        ...
        return 1;
    default: {
        Entry68 *resolved = func_02037b08(modeArg);
        if (resolved == 0) return 0;
        func_02037cd8(resolved, arg1, arg2);
        return 1;
    }
    }
}
