/* CAMPAIGN-PREP candidate for func_02021010 (main, class A) — brief 494.
 ...
 *   confidence: high
 */
/* func_02021010: Fill32 the work area, copy 7 words from a source table into
 * field +0x18 of consecutive 4-byte slots, then tail-use slot[0].
 *   Fill32(0, data_02194260, 0x30f4);
 *   for (i=0; i<7; i++) ((int*)data_02194260)[i+6] = data_020c6798[i];
 *   func_02021ae4(((int*)data_02194260)[6]);
 *   return 1;
...
extern int  data_020c6798[];
extern void func_02021ae4(int);

int func_02021010(void) {
    int *p = data_02194260;
    int i;
    Fill32(0, p, 0x30f4);
    for (i = 0; i < 7; i++) {
        p[i + 6] = data_020c6798[i];
    }
    func_02021ae4(p[6]);
    return 1;
}
