/* func_ov006_021b2704: per-state "enter" — probe two subsystems, run the
 * shared setup pass over three subobjects interleaved with the two state-
 * specific init calls, then mark the status word ready (=3) iff both probes
 * succeeded. Returns the success bool. One of an 8-member family differing
 * only in (init_a, init_b, status) (ov006_core.h §VERIFIED). */
