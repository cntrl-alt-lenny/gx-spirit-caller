extern int func_02005994(void *key, void *base, int count, int width,
                         int (*cmp)(void *, void *));
extern int func_ov006_021b8f00(unsigned short *a, unsigned short *b);

int func_ov006_021b6e50(char *base, int kind, unsigned short val) {
    int local = 0;
    char *r4;
    int cnt;
    int idx;
    ...
    *(unsigned short *)&local = val;
    ...
    idx = 0;
    if (cnt > 0) {
        idx = func_02005994(&local, r4, (unsigned short)cnt, 4, (int (*)(void *, void *))func_ov006_021b8f00);
    }
