/* func_ov018_021aaa10: draw the active gauge row and return its cell count. The
 ...
 * func_ov018_021aa9a8, mode-1 rows fan out by the kind at st+0 (0xc/0xe ->
 * func_ov018_021aa8d8, 0xf -> func_ov018_021aa95c) while other rows blit the
...
int func_ov018_021aaa10(int a0, int *a1) {
    ...
    ret = func_ov018_021aa9a8(a0, a1, *(int *)(st + mode * 4 + 0x8e0), 0x80, sub);
    if (mode == 1) {
        switch (*(int *)(st + 0x0)) {
        case 0xc:
        case 0xe:
            func_ov018_021aa8d8(0x5000602);
            break;
        case 0xf:
            func_ov018_021aa95c(0x5000602);
            break;
        }
    } else {
        func_020943b0(st + 0x964, 0x5000600, 0x20);
    }
    if (*(int *)(st + 0x0) == 0xc) {
