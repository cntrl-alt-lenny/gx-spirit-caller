...
extern int  func_ov002_021bd514(int b0, int i);

int func_ov002_02287664(struct Self *self) {
    int i;
    for (i = 0; i < 5; i++) {
        char *row = data_ov002_022cf08c + (self->b0 & 1) * 0x868 + i * 0x14;
        if (((struct Ov002Slot *)(row + 0x30))->id != 0 &&
            *(u16 *)(row + 0x38) != 0) {
            if (func_ov002_021bd514(self->b0, i) > 0) return 1;
        }
    }
    return 0;
}
