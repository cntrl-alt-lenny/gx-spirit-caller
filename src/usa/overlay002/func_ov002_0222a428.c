...
extern int func_ov002_021de55c(void *a, int b, int c, int d);
extern int func_ov002_021c1008(int a, int b, int c);
extern int func_ov002_021e267c(unsigned int bit, unsigned short f0, unsigned int k, unsigned int fld);

int func_ov002_0222a428(struct Node518 *self) {
    struct Slot518 *slot;
    ...
    slot = (struct Slot518 *)((char *)data_ov002_022cf08c + playerOff + 0x30 + idx * 20);

    if (field9 != slot->sub * 2 + slot->flag13)
        return 0;

    if (*(u16 *)((char *)data_ov002_022cf0c4 + playerOff + idx * 20) == 0)
        return 0;

    if (func_ov002_021de55c(self, self->f2.bit0, self->f2.idx, 0) == 0)
        goto end;

    v = *(u16 *)((char *)data_ov002_022cf0c2 + (self->f2.bit0 & 1) * 0x868 + self->f2.idx * 20);
    r = func_ov002_021c1008(self->f2.bit0, self->f2.idx, 1);

    func_ov002_021e267c(self->f2.bit0, self->f0, 1,
