...
extern int func_ov002_02286000(int a, int b, int c, int d, void *out);
extern int func_ov002_0227b084(int a, int x, int y);

struct Buf62cc { char pad[0x14]; int x; int y; char tail[0x10]; };

int func_ov002_022861bc(int a, int b, int c, int d, int arg5, int arg6) {
    struct Buf62cc buf;
    int r;
    data_ov002_022cd444 = a;
    r = func_ov002_0227b8c8(a, b, c, arg6);
    if (r != -1) return r;
    func_ov002_02286000(a, b, d, arg5, &buf);
    r = func_ov002_0227b084(a, buf.x, buf.y);
    if (r != -1) return r;
    return buf.x <= buf.y;
}
