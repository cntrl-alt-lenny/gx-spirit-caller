/* func_ov002_02229c10: if the cf17c per-player slot is nonzero, notify
 * (0x21e2818/0x21d87dc), then dispatch on whether func_0202df24 accepts
...
extern void func_ov002_021d86ec(int bit);
extern void func_ov002_021e2728(int player);
extern void func_ov002_021d8198(int bit, void *addr);
extern void func_ov002_021e1304(int bit, int b, int c);
...
int func_ov002_02229c10(arg_02229d00_t *p) {
    if (*(int *)((char *)data_ov002_022cf09c + (BIT0(p->field_02) & 1) * 0x868) == 0) {
        goto done;
    }

    func_ov002_021e2728(BIT0(p->field_02));
    func_ov002_021d86ec(BIT0(p->field_02));

    if (func_0202df24(
            LOW13(*(int *)(data_ov002_022cf08c + (BIT0(p->field_02) & 1) * 0x868 + 0x260)),
            p->field_08) != 0) {
        func_ov002_021d8198(BIT0(p->field_02),
            data_ov002_022cf08c + (BIT0(p->field_02) & 1) * 0x868 + 0x260);
    } else {
        func_ov002_021e1304(BIT0(p->field_02), 1, 0);
    }
done:
    return 0;
}
