/* func_ov002_0228c754: collect the set bits (j=0..10) of `mask` at shift
...
int func_ov002_0228c754(int arg0, unsigned int mask, int (*callback)(int, int)) {
    int buf[11];
    int i;
    int best;
    int max_val;
    ...
    for (i = count - 1; i > 0; i--) {
        int r = (int)func_ov002_021afff0((unsigned int)(i + 1));
        int t = buf[i];
        buf[i] = buf[r];
        buf[r] = t;
    }
