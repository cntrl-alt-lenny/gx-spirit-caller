/* func_ov002_0220dd68: self's +2 halfword carries an asymmetric 6-bit
 * dispatch field (bits 6-11) and a bit0 flag (same shape as
 * func_ov002_0220248c); bail if the 6-bit field is 0x23, else scan a
...
int func_ov002_0220dd68(struct Self *self) {
    ...
    slot = (struct Slot *)(data_ov002_022cf08c + idx + 0x94);
    for (i = 5; i < 10; i++) {
        if (slot->id == 0)
            return 0;
