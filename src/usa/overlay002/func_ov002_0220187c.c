...
extern int func_ov002_021b8dec(void *b);
extern int func_0202b824(int id);
extern int func_ov002_021c1e10(void *self, int player, int idx);
extern int func_ov002_022017dc(void *b, int player, int idx);

int func_ov002_0220187c(void *a, void *b) {
    struct F2Cat *b_cat;
    int i, j;
    ...
    if (func_0202b824(*(u16 *)b) != 0x16) {
        goto fail;
    }

    for (i = 0; i < 2; i++) {
        for (j = 0; j < 5; j++) {
            if (func_ov002_021c1e10(a, i, j) != 0) {
                if (func_ov002_022017dc(b, i, j) != 0) {
                    return 1;
                }
            }
        }
