...
extern void func_ov002_021e1304(int x, int one, int match);
extern void func_ov002_021e2b6c(void);

int func_ov002_02234800(void *a) {
    halfword_bit0_t *h = (halfword_bit0_t *)((char *)a + 2);
    int i;
    func_ov002_021e2a4c();
    for (i = 0; i < 2; i++) {
        int x = data_ov002_022d008c.field_cec ^ i;
        int match = (x == h->bit0) ? 0 : 1;
        func_ov002_021e1304(x, 1, match);
    }
    func_ov002_021e2b6c();
    return 0;
}
