/* func_ov002_02294c4c: gate on func_ov002_021bb82c(self->b0, 0xfbc), then
 ...
 * return 1 if func_ov002_021c83c8(1-self->b0, idx) <= 0x9c4 (C-39f
...
extern int func_ov002_021bb82c(int b0, int c);
extern int func_ov002_021c83c8(int a, int b);

int func_ov002_02294c4c(SelfB0v2 *self) {
    int idx;
    int byteOff;

    if (func_ov002_021bb82c(self->b0, 0xfbc) == 0)
        return 0;

    for (idx = 0, byteOff = 0; idx < 5; idx++, byteOff += 0x14) {
        char *row = data_ov002_022cf08c + ((1 - self->b0) & 1) * 0x868;
        ...
        if (func_ov002_021c83c8(1 - self->b0, idx) <= 0x9c4)
            return 1;
    }
    return 0;
}
