 ...
 *
 * Key globals:
 *   data_ov012_021cc6dc — the main controller block. Referenced MANY times
 *     across calls, so the orig binds it ONCE into a callee-saved reg → bind
 *     `char *d = data_ov012_021cc6dc;` at the top of such funcs. Fields: [0x15c]
 *     mode, [0x160] saved-slot (low nibble), [0x17c] map ptr, [0x180] task,
 *     [0x174..] per-corner offsets.
 *   data_ov012_021cc6a0 — mode/config block ([0x198] map mode, [0x1bc] task-armed,
