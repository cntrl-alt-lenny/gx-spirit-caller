...
class TestAsmVoidCounter(unittest.TestCase):
    def test_asm_void_spellings_are_detected(self):
        assert has_asm_void("asm void Foo(void) { nop; }")
        assert has_asm_void("asm { mov r0, r1 }")

    def test_comments_and_literals_are_not_detected(self):
        assert not has_asm_void('/* asm void fake(void) */ const char *s = "asm {";')

    def test_text_bytes_for_sources_reads_only_selected_tus(self):
    ...
    def test_format_report_is_pr_summary_ready(self):
