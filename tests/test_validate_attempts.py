...
# Split into two classes rather than one: tests/test_live_cardinality_guard.py
# treats a whole class as "live" (real repo-state) the moment ANY method's
# source contains a marker like `audit_file(`, and then requires every
# sibling method's `len(x) == <int>` assertions to justify themselves against
# live data. Mixing the synthetic-row tests below into the same class as the
# committed-ledger tests would make the guard flag their literal-count
# assertions as false positives -- they build fully synthetic rows via
# `_row`/`_audit` and never touch live data. Keep the live/synthetic
# distinction as two classes so the guard's per-test granularity holds.
class TestValidateAttemptsSynthetic(unittest.TestCase):
    def test_contradiction_families_are_detected(self) -> None:
    ...
    def test_new_legitimate_c_lever_ship_stays_green(self) -> None:
    ...
    def test_empty_c_lever_exemption_list_fails_loudly(self) -> None:
    ...
    def test_module_and_text_size_ground_truth_are_checked(self) -> None:
    ...
    def test_attempts_accepts_blank_or_integer_and_rejects_other_values(self) -> None:
    ...
    def test_legacy_row_without_attempts_is_treated_as_blank(self) -> None:
        row = _row()
        row.pop("attempts", None)
        assert not _audit(row).schema_errors

    def test_shape_migration_and_conflict_are_distinguished(self) -> None:
    ...
    def test_ship_coverage_requires_a_shipped_event_for_each_flip(self) -> None:
    ...
    def test_ship_coverage_history_shape_rejects_empty_snapshot(self) -> None:
        try:
...
class TestValidateAttemptsLiveLedger(unittest.TestCase):
    ...
    def test_ship_coverage_history_is_not_vacuous(self) -> None:
